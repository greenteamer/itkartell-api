...
module.exports = {
  ...
  fetchAll: (params) => {
    // Get model hook
    const hook = strapi.hook[Project.orm];
    ...
    return Project.query(function(qb) {
      ...
      hook.load().generateMatchStage(qb)(Project, filters);
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Project.forge(params).save(data);
  ...
  remove: async (params) => {
    ...
    await Project.updateRelations(params);
  ...
  search: async (params) => {
    ...
    const associations = Project.associations.map(x => x.alias);
    const searchText = Object.keys(Project._attributes)
      .filter(attribute => attribute !== Project.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['string', 'text'].includes(Project._attributes[attribute].type));

    const searchNoText = Object.keys(Project._attributes)
      .filter(attribute => attribute !== Project.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Project._attributes[attribute].type));

    const searchInt = Object.keys(Project._attributes)
      .filter(attribute => attribute !== Project.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['integer', 'decimal', 'float'].includes(Project._attributes[attribute].type));

    const searchBool = Object.keys(Project._attributes)
      .filter(attribute => attribute !== Project.primaryKey && !associations.includes(attribute))
      .filter(attribute => ['boolean'].includes(Project._attributes[attribute].type));
